    ...
    if (tab === PAYMENT_TAB) {
      if (mode === 'add') {
        ...
        try {
          ...
          setPaymentsPage(1);
          notif({ severity: 'success', message: 'Pagamento criado com sucesso!' });
        } catch (error) {
          console.error('Error creating payment:', error);
          UserSession.verifyAuthorize(navigate, error?.status);
          errorMessage = ErrorHandler(error);
          notif({ severity: 'error', message: errorMessage || 'Erro ao criar pagamento.' });
        }
        ...
        try {
          ...
          setPayments(await HRService.getAllPayments());
          notif({ severity: 'success', message: 'Pagamento atualizado com sucesso!' });
        } catch (error) {
          console.error('Error editing payment:', error);
          UserSession.verifyAuthorize(navigate, error?.status);
          errorMessage = ErrorHandler(error);
          notif({ severity: 'error', message: errorMessage || 'Erro ao atualizar pagamento.' });
        }
      }
    } else {
      if (mode === 'add') {
        try {
          ...
          setJobChangesPage(1);
          notif({ severity: 'success', message: 'Mudança de função criada com sucesso!' });
        } catch (error) {
          console.error('Error creating job change:', error);
          UserSession.verifyAuthorize(navigate, error?.status);
          errorMessage = ErrorHandler(error);
          notif({ severity: 'error', message: errorMessage || 'Erro ao criar mudança de função.' });
        }
      } else if (editIndex != null) {
        ...
        try {
          ...
          setJobChanges(await HRService.getAllMovements());
          notif({ severity: 'success', message: 'Mudança de função atualizada com sucesso!' });
        } catch (error) {
          console.error('Error editing job change:', error);
          UserSession.verifyAuthorize(navigate, error?.status);
          errorMessage = ErrorHandler(error);
          notif({ severity: 'error', message: errorMessage || 'Erro ao atualizar mudança de função.' });
        }
      }
    }
    closeDialog();
  ...
  const handleDelete = async (indexInPage) => {
    ...
    if (tab === PAYMENT_TAB) {
      try {
        ...
        UserSession.verifyAuthorize(navigate, error?.status);
        errorMessage = ErrorHandler(error);
        notif({ severity: 'error', message: errorMessage || 'Erro ao apagar pagamento.' });
      }
    } else {
      try {
        ...
        notif({
          severity: 'error',
          message: errorMessage || 'Erro ao apagar mudança de função.',
        });
      }
    }
  };
                    ...
                    <TableCell>
                      <Stack direction="row" spacing={1}>
                        ...
                        <Popups
                          ...
                          onConfirm={async () => {
                            await handleDelete(idx);
                            // ✅ Notificação de sucesso aqui (Opção B)
                            notif({
                              severity: 'success',
                              message: 'Mudança de função apagada com sucesso!',
                            });
                          }}
                        >
                          <IconButton
